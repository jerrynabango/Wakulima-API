...
class Order(models.Model):
    ...
    order_number = models.CharField(
        _("order number"), max_length=20, unique=True, editable=False
    )
    ...
    delivery_fee = models.DecimalField(
        _("delivery fee"), max_digits=10, decimal_places=2, default=0
    )
    tax = models.DecimalField(
        _("tax"), max_digits=10, decimal_places=2, default=0
    )
    ...
    payment_response = models.JSONField(
        _("payment response"), default=dict, blank=True
    )
    ...
    cancelled_at = models.DateTimeField(
        _("cancelled at"), blank=True, null=True
    )
    ...
    def can_refund(self):
        ...
        return (
            ...
            and self.order_status
            not in [self.OrderStatus.REFUNDED, self.OrderStatus.CANCELLED]
        )
...
class OrderActivity(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    order = models.ForeignKey(
        Order, on_delete=models.CASCADE, related_name="activities"
    )
    activity_type = models.CharField(
        max_length=30, choices=ActivityType.choices
    )
