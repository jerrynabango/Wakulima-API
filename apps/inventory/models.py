...
class Inventory(models.Model):
    ...
    warehouse_location = models.CharField(
        _("warehouse location"), max_length=200, blank=True
    )
    shelf_number = models.CharField(
        _("shelf number"), max_length=50, blank=True
    )

    # Timestamps
    last_updated = models.DateTimeField(_("last updated"), auto_now=True)
    last_restocked = models.DateTimeField(
        _("last restocked"), blank=True, null=True
    )
    ...
    def __str__(self):
        return (
            f"{self.product.name} - {self.quantity} {self.product.unit_type}"
        )
...
class StockAlert(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    inventory = models.ForeignKey(
        Inventory, on_delete=models.CASCADE, related_name="alerts"
    )
    alert_type = models.CharField(
        _("alert type"), max_length=20, choices=AlertType.choices
    )
...
class InventoryReservation(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    inventory = models.ForeignKey(
        Inventory, on_delete=models.CASCADE, related_name="reservations"
    )
