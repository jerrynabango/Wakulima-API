...
class InventoryService:
    ...
    @transaction.atomic
    def update_stock(
        product, user, new_quantity, reason="Manual stock update"
    ):
        """
        ...
        logger.info(f"Stock updated for {
                product.name} by {
                user.email}: {old_quantity} -> {new_quantity}")
    ...
    def adjust_stock(product, user, quantity, adjustment_type, reason):
        ...
        if adjustment_type == "add":
            ...
            if quantity > inventory.quantity:
                return {
                    "success": False,
                    "message": f"Cannot subtract {quantity}. Only {
        ...
        logger.info(f"Stock adjusted for {
                product.name} by {
                user.email}: {old_quantity} -> {
                inventory.quantity}")

        return {"success": True, "inventory": inventory, "message": message}
    ...
    def resolve_stock_alert(alert, user):
        ...
        logger.info(f"Stock alert resolved for {
                alert.inventory.product.name} by {
                user.email}")
    ...
    def get_low_stock_report(user):
        ...
        return Inventory.objects.filter(
            product__farmer=user, quantity__lte=F("minimum_stock")
        )
    ...
    def get_stock_summary(user):
        ...
        return {
            ...
            "healthy_stock_count": total_products
            - low_stock_count
            - out_of_stock_count,
        }
...
class InventoryReservationService:
    ...
    def create_reservation(inventory, order_id, quantity, expires_minutes=30):
        ...
        if quantity > inventory.available_quantity:
            return {
                "success": False,
                "message": f"Insufficient stock. Available: {
        ...
        logger.info(
            f"Reservation created for order {order_id}: {quantity} units"
        )
    ...
    def confirm_reservation(order_id):
        ...
        logger.info(
            f"Confirmed {confirmed_count} reservations for order {order_id}"
        )
    ...
    def cancel_reservation(order_id):
        ...
        logger.info(
            f"Cancelled {cancelled_count} reservations for order {order_id}"
        )
