...
from apps.inventory.models import Inventory, InventoryReservation, StockAlert
    ...
    for inventory in low_stock_inventory:
        alert, created = StockAlert.objects.get_or_create(
            ...
            defaults={
                "message": f"Low stock alert: {
                    inventory.product.name} has {
                    inventory.quantity} {
                    inventory.product.unit_type} remaining",
        ...
        if created:
            ...
            send_low_stock_alert.delay(
                inventory.product.id, inventory.product.farmer.id
            )

    return {
        "alerts_created": alerts_created,
        "low_stock_count": low_stock_inventory.count(),
    }
...
def generate_inventory_report(farmer_id):
    ...
    try:
        ...
        total_value = sum(
            float(i.quantity) * float(i.product.price) for i in inventory
        )
            ...
            "products": [
                {
                    ...
                    "value": round(
                        float(inv.quantity) * float(inv.product.price), 2
                    ),
                    "status": inv.status,
                }
