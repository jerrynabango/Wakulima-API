...
class AuthService:
    ...
    def _get_user_data_dict(user):
        ...
        return {
            ...
            "farmer_requested_at": (
                user.farmer_requested_at.isoformat()
                if user.farmer_requested_at
                else None
            ),
        }
    ...
    def send_otp_email(user, otp, purpose):
        ...
        html_message = render_to_string(
            "accounts/emails/otp_email.html", context
        )
    ...
    def verify_otp(user, otp, purpose):
        ...
        if user.otp_purpose != purpose:
            return {
                "success": False,
                "message": "Invalid OTP for this purpose",
            }
    ...
    def request_farmer_upgrade(user, request):
        ...
        if user.farmer_request_status == User.FarmerRequestStatus.PENDING:
            return {
                "success": False,
                "message": "You already have a pending farmer upgrade request",
            }
        ...
        if result["success"]:
            ...
            user.save(
                update_fields=["farmer_request_status", "farmer_requested_at"]
            )

            # Log activity - ensure details are JSON serializable
    ...
    def verify_farmer_upgrade(user, otp, request):
        ...
        AuthService.log_user_activity(
            ...
            {
                "old_role": old_role,
                "new_role": User.Role.FARMER,
                "user_id": str(user.id),
            },
        )

        # Send welcome email for farmers (handle errors gracefully)
        try:
            ...
            logger.error(f"Failed to send farmer welcome email: {str(e)}")
        ...
        return {
            ...
            "user_data": AuthService._get_user_data_dict(user),
        }
    ...
    def get_user_profile(user):
        ...
        return AuthService._get_user_data_dict(user)

    @staticmethod
    def update_user_profile(user, data):
        """
        Update user profile information
        Returns: {'success': bool, 'user_data': dict, 'updated_fields': list}
        """
        updated_fields = []
        ...
        if (
            ...
            and data["phone_number"] != user.phone_number
        ):
    ...
    def reset_password_with_otp(
        email, otp, new_password, confirm_password, request=None
    ):
        """
        ...
        AuthService.log_user_activity(
            ...
            request or {},
            {"action": "password_reset_completed", "user_id": str(user.id)},
        )
    ...
    def log_user_activity(user, activity_type, request, details=None):
        """Log user activity - ensures details are JSON serializable"""
        # Ensure details is a dict and contains only serializable data
        if details is None:
            details = {}
        ...
        for key, value in details.items():
            if isinstance(value, User):
                clean_details[key] = str(value.id)
        ...
        try:
            ...
            logger.error(f"Failed to log user activity: {
                    str(e)} - Details: {clean_details}")
    ...
    def log_login(user, request):
        ...
        AuthService.log_user_activity(
            ...
            request,
            {"action": "login", "user_id": str(user.id)},
        )
    ...
    def log_logout(user, request):
        ...
        AuthService.log_user_activity(
            ...
            request,
            {"action": "logout", "user_id": str(user.id)},
        )
