...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    phone_number = models.CharField(
        ...
        validators=[
            RegexValidator(
                regex=r"^\+?254\d{9}$",
                message=_(
                    "Phone number must be in Kenyan format (e.g., +254712345678)"
                ),
            )
        ],
        blank=True,
    ...
    farmer_requested_at = models.DateTimeField(
        _("farmer requested at"), blank=True, null=True
    )
    ...
    )  # 'password_reset', 'farmer_upgrade', 'email_verification'
                ...
                if self.otp_last_request_at > one_hour_ago:
                    return (
                        ...
                        f"Too many attempts. Please wait {
                            ...
                            (
                                timezone.now() -
                                self.otp_last_request_at).seconds //
                            60} minutes",
                    )
        return True, "OK"
...
class UserActivityLog(models.Model):
    ...
    class ActivityType(models.TextChoices):
        ...
        PROFILE_PICTURE_UPLOAD = "profile_picture_upload", _(
            "Profile Picture Upload"
        )
        PROFILE_PICTURE_DELETE = "profile_picture_delete", _(
