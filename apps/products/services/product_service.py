...
class ProductService:
    ...
    def get_products_for_user(user, filters=None):
        ...
        if not user.is_authenticated or user.role != "farmer":
            ...
            queryset = Product.objects.filter(
                status=Product.ProductStatus.ACTIVE, is_available=True
            )
        elif user.is_farmer:
        ...
        return queryset.select_related("category", "farmer").prefetch_related(
            "images"
        )
    ...
    def create_product(user, product_data, images=None):
        ...
        try:
            ...
            if Product.objects.filter(
                name__iexact=product_data["name"], farmer=user
            ).exists():
                return {
                    "success": False,
                    "errors": {
            ...
            # Create product
            product = Product.objects.create(farmer=user, **product_data)
            ...
            logger.info(f"Product {
                    product.name} created by farmer {
                    user.email}")
            ...
            if new_images:
                for index, image in enumerate(new_images):
                    ProductImage.objects.create(
                        ...
                        image=image,
                        is_primary=(
                            index == 0 and not product.images.exists()
                        ),
    ...
    def delete_product(product, user):
        ...
        try:
            ...
            logger.info(f"Product {
                    product.name} marked as discontinued by {
                    user.email}")
    ...
    def update_stock(product, user, new_quantity, reason=None):
        ...
        try:
            ...
            InventoryHistory.objects.create(
                ...
                new_quantity=new_quantity,
                reason=reason
                or f"Manual stock update from {old_quantity} to {new_quantity}",
            )

            logger.info(f"Stock updated for {
                    product.name}: {old_quantity} -> {new_quantity}")
            ...
            return {"success": False, "message": f"Failed to update stock: {
                    str(e)}"}
        ...
        try:
            ...
            if ProductReview.objects.filter(
                ...
                return {
                    "success": False,
                    "errors": {
                        "user": "You have already reviewed this product"
                    },
                }
