...
class Category(models.Model):
    ...
    icon = models.CharField(_('icon'), max_length=50, blank=True, help_text="Font awesome icon class")
...
class Product(models.Model):
    ...
    # Stock and Inventory
    unit_type = models.CharField(_('unit type'), max_length=10, choices=UnitType.choices, default=UnitType.KILOGRAM)
    ...
    # Quality and Origin
    quality_grade = models.CharField(_('quality grade'), max_length=10, choices=QualityGrade.choices, default=QualityGrade.STANDARD)
    origin = models.CharField(_('origin'), max_length=200, blank=True, help_text="Farm or region of origin")
    ...
    is_locally_sourced = models.BooleanField(_('locally sourced'), default=True)
    ...
    weight = models.DecimalField(_('weight (kg)'), max_digits=8, decimal_places=2, blank=True, null=True)
    length = models.DecimalField(_('length (cm)'), max_digits=6, decimal_places=2, blank=True, null=True)
    width = models.DecimalField(_('width (cm)'), max_digits=6, decimal_places=2, blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.quantity <= 0:
            ...
            if self.status == ProductStatus.OUT_OF_STOCK and self.quantity > 0:
...
class InventoryHistory(models.Model):
    ...
    change_type = models.CharField(_('change type'), max_length=20, choices=ChangeType.choices)
    quantity_change = models.DecimalField(_('quantity change'), max_digits=10, decimal_places=2)
    ...
    reference_id = models.CharField(_('reference ID'), max_length=100, blank=True, help_text="Order ID or other reference")
    created_at = models.DateTimeField(_('created at'), auto_now_add=True)
