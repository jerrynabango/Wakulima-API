...
def check_low_stock(sender, instance, **kwargs):
    ...
    if instance.pk:
        try:
            ...
            if old_instance.quantity > instance.minimum_stock and instance.quantity <= instance.minimum_stock:
                logger.warning(f"Low stock alert triggered for product: {instance.name} (Stock: {instance.quantity})")
                
                # Create low stock alert via notification service
                notification_service = NotificationService()
                notification_service.send_low_stock_alert(instance, instance.farmer)
...
def log_inventory_change(sender, instance, created, **kwargs):
    ...
    """
    if created:
        logger.info(f"Inventory change recorded: {instance.change_type} - {instance.quantity_change} units for {instance.product.name}")
        
        # For order-related inventory changes, trigger notifications
        if instance.change_type in [InventoryHistory.ChangeType.ORDER, InventoryHistory.ChangeType.RETURN]:
        ...
        try:
            ...
            inventory, created = Inventory.objects.get_or_create(
                ...
                defaults={
                    ...
                    'reorder_point': 10,
                    'reorder_quantity': 20,
                    'status': Inventory.StockStatus.IN_STOCK if instance.quantity > 0 else Inventory.StockStatus.OUT_OF_STOCK
                }
            )
            logger.info(f"Inventory record created for new product: {instance.name}")
        except ImportError:
            # Inventory app not installed yet
            logger.info(f"Inventory app not available - skipping inventory creation for {instance.name}")
        except Exception as e:
            logger.error(f"Failed to create inventory for {instance.name}: {str(e)}")
    ...
    if not kwargs.get('created', False):  # Only for updates, not new products
        try:
            ...
            if inventory.quantity != instance.quantity:
                ...
                logger.info(f"Inventory updated for product {instance.name}: new quantity {instance.quantity}")
            ...
            try:
                ...
                logger.info(f"Inventory created for existing product: {instance.name}")
            except ImportError:
                pass
        except Exception as e:
            logger.error(f"Failed to update inventory for {instance.name}: {str(e)}")
