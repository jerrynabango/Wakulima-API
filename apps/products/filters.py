...
class ProductFilter(filters.FilterSet):
    ...
    class Meta:
        model = Product
        fields = [
            'category', 'is_organic', 'is_locally_sourced', 
            'quality_grade', 'unit_type', 'status'
        ]
    
    def filter_by_min_rating(self, queryset, name, value):
        """Filter products by minimum average rating"""
        return queryset.annotate(avg_rating=models.Avg('reviews__rating')).filter(avg_rating__gte=value)
