...
from .choices import NotificationType, NotificationCategory, NotificationPriority, EmailTemplateType, SMSTemplateType
...
class Notification(models.Model):
    ...
    notification_type = models.CharField(_('type'), max_length=20, choices=NotificationType.choices)
    category = models.CharField(_('category'), max_length=20, choices=NotificationCategory.choices)
    priority = models.CharField(_('priority'), max_length=10, choices=NotificationPriority.choices, default=NotificationPriority.NORMAL)
    ...
    reference_id = models.CharField(_('reference ID'), max_length=100, blank=True, help_text="Order ID, Payment ID, etc.")
    ...
    def __str__(self):
        return f"{self.notification_type} to {self.user.email}: {self.title[:50]}"
...
class EmailLog(models.Model):
    ...
    from_email = models.EmailField(_('from email'), default=settings.DEFAULT_FROM_EMAIL)
    subject = models.CharField(_('subject'), max_length=200)
    template_type = models.CharField(_('template type'), max_length=50, choices=EmailTemplateType.choices)
    ...
    # Status
    sendgrid_message_id = models.CharField(_('SendGrid message ID'), max_length=100, blank=True)
    status = models.CharField(_('status'), max_length=50, default='pending')  # pending, sent, failed, delivered, opened, clicked
    delivered_at = models.DateTimeField(_('delivered at'), blank=True, null=True)
...
class SMSLog(models.Model):
    ...
    message = models.TextField(_('message'))
    template_type = models.CharField(_('template type'), max_length=50, choices=SMSTemplateType.choices)
    
    # Status
    message_id = models.CharField(_('message ID'), max_length=100, blank=True)
    status = models.CharField(_('status'), max_length=50, default='pending')  # pending, sent, failed, delivered
