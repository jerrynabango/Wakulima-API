...
class AfricaTalkingSMSService:
    ...
    def __init__(self):
        ...
        if self.username and self.api_key:
            ...
            logger.info(f"Africa's Talking SMS service initialized - Sandbox: {self.sandbox}")
        else:
            logger.warning("Africa's Talking credentials not configured - check your .env file")
            self.sms = None
    
    def send_sms(self, to_phone, message, template_type, reference_id=None, metadata=None):
        """
        ...
        try:
            ...
            # Send SMS
            response = self.sms.send(message, [formatted_phone], sender_id=self.sender_id)
            ...
            if response and response.get('SMSMessageData'):
                ...
                if recipients:
                    ...
                    logger.info(f"SMS sent to {formatted_phone} from {self.sender_id} - Template: {template_type}")
...
class SMSTemplateService:
    ...
    def get_order_confirmation_message(order):
        ...
        return f"Wakulima: Order #{order.order_number} confirmed! Amount: KES {order.total}. We'll notify you when shipped."
    ...
    def get_order_shipped_message(order):
        ...
        return f"Wakulima: Order #{order.order_number} shipped!{tracking} Expected delivery in 2-3 days."
    
    @staticmethod
    def get_order_delivered_message(order):
        """Generate order delivered SMS"""
        return f"Wakulima: Order #{order.order_number} delivered! Thank you for shopping with us."
    
    @staticmethod
    def get_payment_received_message(payment):
        """Generate payment received SMS"""
        return f"Wakulima: Payment of KES {payment.amount} received for Order #{payment.order.order_number}. Thank you!"
    ...
    def get_low_stock_message(product):
        ...
        return f"Wakulima Alert: {product.name} is running low! Current stock: {product.quantity} {product.unit_type}. Restock soon."
