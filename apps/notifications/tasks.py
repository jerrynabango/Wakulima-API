...
def send_order_confirmation_notification(order_id):
    ...
    try:
        ...
        logger.info(
            f"Order confirmation notification sent for order {order_id}: {result}"
        )
        return result
    except Exception as e:
        logger.error(
            f"Failed to send order confirmation for order {order_id}: {str(e)}"
        )
        return {"error": str(e)}
...
def send_payment_received_notification(payment_id):
    ...
    try:
        ...
        logger.info(
            f"Payment received notification sent for payment {payment_id}: {result}"
        )
        return result
    except Exception as e:
        logger.error(
            f"Failed to send payment notification for payment {payment_id}: {
                str(e)}"
        )
        return {"error": str(e)}
...
def cleanup_old_notifications():
    ...
    deleted_notifications = Notification.objects.filter(
        created_at__lt=cutoff_date, read=True
    ).delete()[0]
    ...
    deleted_sms = SMSLog.objects.filter(sent_at__lt=cutoff_date).delete()[0]

    logger.info(
        f"Cleaned up: {deleted_notifications} notifications, {deleted_emails} emails, {deleted_sms} SMS"
    )
