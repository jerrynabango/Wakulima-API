...
class PaymentService:
    ...
    def check_order_payment_status(order):
        ...
        return order.payments.filter(status=Payment.PaymentStatus.SUCCESS).exists()
    
    @staticmethod
    @transaction.atomic
    def create_payment_record(order, user, amount, currency, gateway, **kwargs):
    ...
    def get_user_payments(user, limit=100):
        ...
        return Payment.objects.filter(user=user).select_related('order')[:limit]
    ...
    def update_payment_status(payment, status, **kwargs):
        ...
        if status == Payment.PaymentStatus.SUCCESS and old_status != status:
            ...
            OrderActivity.objects.create(
                ...
                description=f"Payment confirmed via {payment.gateway} - Amount: {payment.amount} {payment.currency}",
...
class MpesaPaymentService:
    ...
    def process_webhook(webhook_data, client_ip):
        ...
        if not handler.validate_signature(
            ...
            logger.error(f"M-Pesa webhook validation failed for IP: {client_ip}")
            return {
                'success': False,
                'result_code': 1,
        ...
        return {
            ...
            'result_desc': result.get('result_desc', 'Success' if result.get('success') else 'Failed')
        }
