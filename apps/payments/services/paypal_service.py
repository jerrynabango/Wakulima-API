...
class PayPalService:
    ...
    def create_order(
        ...
        """Create PayPal order"""
        try:
            # Determine currency (PayPal requires USD for sandbox in some
            # regions)
            currency = (
                "USD"  # Force USD for simplicity, or implement conversion
            )
            ...
            logger.info(
                f"PayPal order created for payment {payment.id} - Order ID: {order.id}"
            )
    ...
    def capture_order(self, payment: Payment, payer_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if capture.success():
                ...
                logger.info(f"PayPal payment captured for payment {
                ...
                error_message = (
                    capture.error.get("message", "Unknown error")
                    if capture.error
                    else "Capture failed"
                )
                ...
                logger.error(f"PayPal capture failed for payment {
                        payment.id}: {
                        capture.error}")
    ...
    def refund_payment(
        self, payment: Payment, amount: Decimal, reason: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if refund.success():
                ...
                logger.info(
                    f"PayPal refund processed for payment {payment.id} - Refund ID: {refund.id}"
                )
                ...
                error_message = (
                    refund.error.get("message", "Unknown error")
                    if refund.error
                    else "Refund failed"
                )
                logger.error(f"PayPal refund failed for payment {
                        payment.id}: {
                        refund.error}")
