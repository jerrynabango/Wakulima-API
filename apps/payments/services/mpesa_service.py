    ...
    def stk_push(self, payment: Payment, phone_number: str) -> Dict[str, Any]:
        ...
        try:
            ...
            payload = {
                ...
                "AccountReference": payment.order.order_number[:12],  # Max 12 chars
            ...
            payment.save(update_fields=['mpesa_checkout_request_id', 'request_data'])
            
            logger.info(f"M-Pesa STK Push initiated for payment {payment.id} - Checkout ID: {data.get('CheckoutRequestID')}")
        ...
        try:
            ...
            if result_code == '0':
                ...
                if payment.status != Payment.PaymentStatus.SUCCESS:
                    ...
                    logger.info(f"M-Pesa payment confirmed via query for payment {payment.id}")
                ...
                if payment.status != Payment.PaymentStatus.CANCELLED:
                    ...
                    payment.mpesa_result_desc = result_desc
                    payment.save()
                    logger.info(f"M-Pesa payment cancelled for payment {payment.id}")
                ...
                if payment.status != Payment.PaymentStatus.FAILED:
                    ...
                    payment.mpesa_result_desc = result_desc
                    payment.save()
                    logger.warning(f"M-Pesa payment failed for payment {payment.id}: {result_desc}")
