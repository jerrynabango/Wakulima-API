...
def payment_status_change_handler(sender, instance, created, **kwargs):
    ...
    if not created:
            ...
            try:
                ...
                if old_instance.status != instance.status:
                    ...
                    if instance.status == Payment.PaymentStatus.SUCCESS:
                        ...
                        description = f"Payment successful - Amount: {instance.amount} {instance.currency}"
                    elif instance.status == Payment.PaymentStatus.FAILED:
                        activity_type = 'payment_failed'
                        description = f"Payment failed - Gateway: {instance.gateway}"
                    elif instance.status == Payment.PaymentStatus.REFUNDED:
                        activity_type = OrderActivity.ActivityType.REFUND_COMPLETED
                        description = f"Payment refunded - Amount: {instance.amount} {instance.currency}"
                    ...
                    logger.info(f"Payment {instance.id} status changed: {old_instance.status} -> {instance.status}")
        ...
        OrderActivity.objects.create(
            ...
            description=f"Payment initiated - Amount: {instance.amount} {instance.currency} via {instance.gateway}",
            performed_by=instance.user,
            ip_address='system',
            new_status=instance.status
        )
...
def payment_pre_save_handler(sender, instance, **kwargs):
    ...
    if instance.transaction_id and instance.pk:
        ...
        if existing:
            logger.warning(f"Duplicate transaction_id detected: {instance.transaction_id}")
...
def webhook_saved_handler(sender, instance, created, **kwargs):
    ...
    if created:
        logger.info(f"New webhook received: {instance.gateway} - {instance.event_type}")
