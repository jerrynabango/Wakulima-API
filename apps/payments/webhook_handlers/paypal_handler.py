...
class PayPalWebhookHandler(BaseWebhookHandler):
    ...
    def validate_signature(
        ...
        if settings.DEBUG or settings.PAYPAL_MODE == "sandbox":
            if not settings.DEBUG:
                ...
                logger.info(
                    "Sandbox environment - performing PayPal signature validation"
                )
            else:
                logger.info(
                    "Development environment - skipping PayPal signature validation"
                )
                return True
        ...
        if not headers or not raw_body:
            logger.error(
                "Missing headers or raw body for PayPal webhook validation"
            )
            return False
        ...
        verification_url = f"{
            self.api_url}/v1/notifications/verify-webhook-signature"
        ...
        try:
            ...
            if verification_status == "SUCCESS":
                ...
                logger.error(
                    f"PayPal webhook verification failed: {verification_status}"
                )
                return False
    ...
    def _get_access_token(self) -> str:
        ...
        try:
            response = requests.post(
                auth_url, headers=headers, data=data, timeout=10
            )
            response.raise_for_status()
    ...
    def process_event(self, webhook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.save_webhook_record(
            event_type=event_type, payload=webhook_data, processed=False
        )
        ...
        if handler:
            ...
            self._mark_webhook_processed(
                event_id, result.get("success", False)
            )
            return result
        else:
    ...
    def _is_duplicate_webhook(self, event_id: str) -> bool:
        ...
        if PaymentWebhook.objects.filter(
            gateway="paypal", payload__id=event_id, processed=True
        ).exists():
    ...
    def _handle_payment_capture_completed(
        ...
        seller_receivable_breakdown = resource.get(
        ...
        net_amount = seller_receivable_breakdown.get("net_amount", {}).get(
        ...
        OrderActivity.objects.create(
            ...
            description=f"PayPal payment captured - Capture ID: {capture_id} - Amount: {
                amount.get('value')} {
                amount.get('currency_code')} (Net: {net_amount})",
            performed_by=payment.user,
            ip_address="webhook",
        )

        logger.info(f"PayPal payment captured: {
                payment.id} - Capture ID: {capture_id}")
    ...
    def _handle_payment_capture_denied(
        self, webhook_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if payment:
            ...
            logger.warning(f"PayPal payment capture denied: {
                    payment.id} - {reason}")

        return {"success": True, "message": "Payment capture denied processed"}
    ...
    def _handle_payment_refunded(
        ...
        if payment:
            ...
            OrderActivity.objects.create(
                ...
                description=f"PayPal refund processed - Refund ID: {refund_id} - Amount: {
                    amount.get('value')} {
            ...
            logger.info(f"PayPal refund processed: {
                    payment.id} - Refund ID: {refund_id}")
    ...
    def _handle_order_approved(
        ...
        return {"success": True, "message": "Order approval processed"}
    ...
    def _handle_order_completed(
        ...
        return {"success": True, "message": "Order completion processed"}
    ...
    def _handle_dispute_created(
        ...
        if payment:
            ...
            OrderActivity.objects.create(
                ...
                description=f"PayPal dispute opened: {reason} - Amount: {
                    amount.get('value')} {
                    amount.get('currency_code')}",
                performed_by=None,
                ip_address="webhook",
            ...
            logger.warning(f"PayPal dispute created for payment {
                    payment.id}: {dispute_id}")

        return {"success": True, "message": "Dispute creation processed"}

    def _handle_dispute_resolved(
        self, webhook_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _handle_authorization_voided(
        self, webhook_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {"success": True, "message": "Authorization voided processed"}
    ...
    def _mark_webhook_processed(self, event_id: str, success: bool):
        ...
        if event_id:
            webhook = PaymentWebhook.objects.filter(
                payload__id=event_id, processed=False
            ).first()
            ...
            if webhook:
                ...
                if success:
                    cache.set(
                        f"paypal_webhook_{event_id}", True, timeout=86400
                    )
    ...
    def _trigger_post_payment_actions(self, order: Order):
        ...
        # Additional post-payment actions can be added here
        logger.info(
            f"Post-payment actions triggered for order {order.order_number}"
        )
