...
from apps.payments.api.views import (
    ...
    PaymentHistoryView,
    PayPalWebhookView,
    QueryMpesaPaymentView,
)

urlpatterns = [
    # M-Pesa routes
    path(
        "payments/mpesa/initiate/<uuid:order_id>/",
    ...
    path(
        "payments/mpesa/query/<uuid:payment_id>/",
        QueryMpesaPaymentView.as_view(),
        name="mpesa-query",
    ),
    # PayPal routes
    path(
        "payments/paypal/initiate/<uuid:order_id>/",
        InitiatePayPalPaymentView.as_view(),
        name="paypal-initiate",
    ),
    path(
        "payments/paypal/capture/<uuid:payment_id>/",
        CapturePayPalPaymentView.as_view(),
        name="paypal-capture",
    ),
    path(
        "payments/paypal/webhook/",
        PayPalWebhookView.as_view(),
        name="paypal-webhook",
    ),
