...
def check_pending_payments():
    ...
    for payment in pending_mpesa:
        try:
            ...
            if cache.get(cache_key):
                logger.info(f"Skipping duplicate query for payment {
                        payment.id}")
                continue
    ...
    for payment in pending_paypal:
        try:
            ...
            if cache.get(cache_key):
                logger.info(f"Skipping duplicate PayPal query for payment {
                        payment.id}")
                continue
            ...
            if result.get("success") and result.get("status") == "COMPLETED":
                if payment.status != Payment.PaymentStatus.SUCCESS:
                    ...
                    logger.info(f"Updated PayPal payment {
                            payment.id} to SUCCESS via scheduled task")
            ...
            logger.error(f"Failed to check PayPal payment {
                    payment.id}: {
                    str(e)}")
            results.append({"payment_id": str(payment.id), "error": str(e)})
        ...
        try:
            if webhook.gateway == Payment.PaymentGateway.MPESA:
                ...
                if result.get("success"):
                    ...
                    webhook.error_message = result.get(
                        "error", "Unknown error"
                    )
                    webhook.save()
                    failed_count += 1
                    logger.error(
                        f"Failed to process M-Pesa webhook {webhook.id}: {result.get('error')}"
                    )
                ...
                if result.get("success"):
                    ...
                    webhook.save()
                    failed_count += 1
                    logger.error(f"Failed to process PayPal webhook {
                            webhook.id}: {
                            result.get('error')}")
...
def cleanup_old_webhooks():
    ...
    deleted_count = PaymentWebhook.objects.filter(
        created_at__lt=cutoff_date, processed=True
    ).delete()[0]
    ...
    logger.info(
        f"Cleaned up {deleted_count} old webhooks and {old_payments} old pending payments"
    )
