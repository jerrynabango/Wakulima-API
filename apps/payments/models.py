...
class Payment(models.Model):
    ...
    gateway = models.CharField(
        _("payment gateway"), max_length=10, choices=PaymentGateway.choices
    )
    ...
    transaction_id = models.CharField(
        _("transaction ID"), max_length=200, blank=True, unique=True, null=True
    )
    gateway_reference = models.CharField(
        _("gateway reference"), max_length=200, blank=True
    )

    # Request/Response data
    request_data = models.JSONField(
        _("request data"), default=dict, blank=True
    )
    response_data = models.JSONField(
        _("response data"), default=dict, blank=True
    )
    webhook_data = models.JSONField(
        _("webhook data"), default=dict, blank=True
    )
    ...
    customer_phone = models.CharField(
        _("customer phone"), max_length=15, blank=True
    )
...
class PaymentWebhook(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    gateway = models.CharField(
        _("gateway"), max_length=10, choices=Payment.PaymentGateway.choices
    )
