            ...
            try:
                ...
                if value > product.quantity:
                    raise serializers.ValidationError(
                        f"Only {product.quantity} {product.unit_type} available in stock"
                    )
...
class AddToCartSerializer(serializers.Serializer):
    ...
    quantity = serializers.DecimalField(max_digits=10, decimal_places=2, min_value=0.01, required=True)
...
class CartSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cart
        fields = (
            'id', 'items', 'total_items', 'subtotal', 'delivery_fee',
            'tax', 'total', 'summary', 'created_at', 'updated_at'
        )
