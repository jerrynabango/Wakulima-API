        ...
        try:
            ...
            if quantity > product.quantity:
                return {
                    ...
                    "message": f"Only {
                        product.quantity} {
                        product.unit_type} available",
                    "errors": {"quantity": f"Max {
                            product.quantity} {
                            product.unit_type}"},
                }
            ...
            if not created:
                ...
                if new_quantity > product.quantity:
                    return {
                        "success": False,
                        "message": f"Cannot add {quantity}. Only {
                            product.quantity -
                            cart_item.quantity} more available",
                        "errors": {"quantity": "Exceeds available stock"},
                    }
                cart_item.quantity = new_quantity
                cart_item.save()

            logger.info(f"Added {quantity} of {
                    product.name} to cart {
                    cart.id}")

            return {
                "success": True,
                "message": f"Added {quantity} {
                    product.unit_type} of {
                    product.name} to cart",
                "cart": cart,
                "cart_item": cart_item,
            }
    ...
    def update_cart_item(user, item_id, quantity):
        ...
        try:
            cart_item = get_object_or_404(
                CartItem, id=item_id, cart__user=user
            )
                ...
                if quantity > cart_item.product.quantity:
                    return {
                        ...
                        "message": f"Only {
                            ...
                            cart_item.product.unit_type} available",
